...
EMSCRIPTEN_KEEPALIVE void *getMains() { return Wrapper_mainsArray; };

EMSCRIPTEN_KEEPALIVE void *getFieldGroups() { return Wrapper_appFieldGroups; }

EMSCRIPTEN_KEEPALIVE void *getUnit() { return &Unit; }

EMSCRIPTEN_KEEPALIVE void *getNil() { return &Nil; }

EMSCRIPTEN_KEEPALIVE void *getTrue() { return &True; }

EMSCRIPTEN_KEEPALIVE void *getFalse() { return &False; }

EMSCRIPTEN_KEEPALIVE void *getJsNull() { return &Json_null; }

EMSCRIPTEN_KEEPALIVE void *allocate(size_t size) { return GC_allocate(true, size); }

EMSCRIPTEN_KEEPALIVE u32 stack_push_frame(void *eval_func) { return (u32) GC_stack_push_frame('W', eval_func); }
...
EMSCRIPTEN_KEEPALIVE f64 readF64(f64 *addr) { return *addr; }

EMSCRIPTEN_KEEPALIVE void writeF64(f64 *addr, f64 value) { *addr = value; }

EMSCRIPTEN_KEEPALIVE void *evalClosure(Closure *c) { return GC_execute(c); }

EMSCRIPTEN_KEEPALIVE void *get_Scheduler_rawSpawn() { return &eval_Scheduler_rawSpawn; }

EMSCRIPTEN_KEEPALIVE void *get_Scheduler_spawn() { return &eval_Scheduler_spawn; }

EMSCRIPTEN_KEEPALIVE void *get_Platform_sendToApp() { return &eval_Platform_sendToApp; }

EMSCRIPTEN_KEEPALIVE void *get_Platform_sendToSelf() { return &eval_Platform_sendToSelf; }

EMSCRIPTEN_KEEPALIVE void *get_sendToApp_revArgs() { return &eval_sendToApp_revArgs; }

EMSCRIPTEN_KEEPALIVE Cons *initializeEffects() { return Platform_initializeEffects(); }
...
EMSCRIPTEN_KEEPALIVE void debugAddrRange(size_t *start, size_t len) { print_heap_range(start, start + len); }

EMSCRIPTEN_KEEPALIVE void debugEvaluatorName(void *addr) {
    safe_printf("\nevaluator %zd: %s\n\n", addr, Debug_evaluator_name(addr));
}

EMSCRIPTEN_KEEPALIVE void debugStackMap() { print_stack_map(); }
...
void *eval_intercept(void *args[]) {
    ...
    Record *wrappedTEA =
            Utils_update(userTEA, 2, (u32[]){FIELD_init, FIELD_view}, (void *[]){wrappedInit, wrappedView});
