...
Cons *Platform_initializeEffects() {
    if (!Platform_subscriptions || !Platform_model) {
        safe_printf("To run a WebAssembly app, you need to pass your program configuration record "
    ...
    Closure *sendToAppFromManager = newClosure(1, 2, eval_sendToApp_revArgs, (void *){viewMetadata});
...
Cons *Platform_setupEffects(Custom *managerProcs, Closure *sendToApp) {
    ...
    for (u32 managerId = 0; managerId < Platform_managers_size; managerId++) {
        ...
        if (config->ctor != MANAGER_EFFECT) {
            ...
            JsRef *jsObject = config->ctor == MANAGER_PORT_IN ? Platform_setupIncomingPort(managerId, port)
...
ManagerConfig *Platform_createManager(void *init, Closure *onEffects, Closure *onSelfMsg, Closure *cmdMap,
                                      Closure *subMap) {
...
void *eval_Platform_leaf(void *args[]) { return newCustom(MANAGER_MSG_LEAF, 2, args); }
...
void *eval_Platform_batch(void *args[]) { return newCustom(MANAGER_MSG_NODE, 1, args); }
...
void *eval_Platform_map(void *args[]) { return newCustom(MANAGER_MSG_MAP, 2, args); }
...
void Platform_gatherEffects(bool isCmd, ManagerMsg *bag, Custom *effectsDict, Cons *taggers) {
    switch (bag->ctor) {
        case MANAGER_MSG_LEAF: {
            ...
            effectsDict->values[home] = Platform_insert(isCmd, effect, effectsDict->values[home]);
            return;
        }
        ...
        default: {
            log_error("ERROR in Platform_gatherEffects, unknown effect constructor %d\n", bag->ctor);
        }
    }
}
...
JsRef *Platform_setupOutgoingPort(size_t managerId, PortConfig *config) {
    ...
    config->onEffects = newClosure(2, 5, eval_Platform_outgoingPortOnEffects, (void *[]){jsCallSubs, config});
...
JsRef *Platform_setupIncomingPort(size_t managerId, PortConfig *config) {
    ...
    config->onEffects = newClosure(1, 4, eval_Platform_incomingPortOnEffects, (void *[]){config});
