...
u16 *String_copy(u16 *to, ElmString *s) { return copy_chars(to, s->words16, s->words16 + code_units(s)); }
...
ptrdiff_t find_reverse(u16 *sub, u16 *str, size_t sub_len, ptrdiff_t str_idx) {
    for (;;) {
        ...
        if (sub_idx < 0)
            return str_idx + 1; // Found it!
        if (str_idx < 0)
            return -1; // Matched some more chars then ran out => not found.
...
ptrdiff_t find_forward(u16 *sub, u16 *str, size_t sub_len, size_t str_len) {
    ...
    for (;;) {
        ...
        if (sub_idx >= sub_len)
            return str_idx - sub_len; // Found it!
        if (str_idx >= str_len)
            return -1; // Reached end without full match
        str_idx = retry_idx; // Only a partial match. Backtrack and try again.
    }
}
...
void *eval_String_split(void *args[]) {
    ...
    do {
        ...
        copy_chars(substr->words16, str->words16 + substr_idx, str->words16 + substr_idx + substr_len);
...
void *eval_String_toFloat(void *args[]) {
    ...
    return isnan(value) ? &g_elm_core_Maybe_Nothing : A1(&g_elm_core_Maybe_Just, newElmFloat(value));
}
