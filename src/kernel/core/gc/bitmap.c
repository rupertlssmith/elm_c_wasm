...
retry:
    ...
    if (free_bitmap_index > 0 && bitmap[free_bitmap_index - 1] != ALL_ONES) {
        ...
        size_t extra = 0;
        if (!(mark_bits & 0xFFFFFFFF00000000))
            extra += 32;
        if (!(mark_bits & 0xFFFF0000FFFF0000))
            extra += 16;
        if (!(mark_bits & 0xFF00FF00FF00FF00))
            extra += 8;
        free_ptr -= extra;
    }
    ...
    if (live_bitmap_index < heap->bitmap_size && bitmap[live_bitmap_index] != ALL_ONES) {
        ...
        size_t extra = 0;
        if (!(mark_bits & 0x00000000FFFFFFFF))
            extra += 32;
        if (!(mark_bits & 0x0000FFFF0000FFFF))
            extra += 16;
        if (!(mark_bits & 0x00FF00FF00FF00FF))
            extra += 8;
        live_ptr += extra;
    }
