...
void *Utils_clone(void *x) {
    Header *h = (Header *) x;
    if (h == pNil || (h->tag == Tag_Custom && custom_params(x) == 0))
        return x;
...
static u32 eq_help(ElmValue *pa, ElmValue *pb, u32 depth, ElmValue **pstack) {
    ...
    if (pa->header_as_u32 != pb->header_as_u32)
        return 0;
    ...
    switch (pa->header.tag) {
        ...
        case Tag_Closure: {
            if (pa->closure.evaluator != pb->closure.evaluator)
                return 0;
        ...
        case Tag_List:
            if (y == pNil)
                ...
                while (1) {
                    ...
                    if (order_head != &g_elm_core_Basics_EQ)
                        return order_head;
                    x = x->cons.tail;
                    y = y->cons.tail;
                    if (x == pNil || y == pNil)
