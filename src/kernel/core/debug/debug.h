...
void Debug_assert(char *file, int line, const char *function, char *expr_text, char *print_text, bool expr,
                  u64 print_value);
...
#define ASSERT_SANITY(elmValuePtr) Debug_assert_sanity(__FILE__, __LINE__, __FUNCTION__, #elmValuePtr, elmValuePtr)
...
PerfCounter *perf_section_start(u16 index, const char *function, u32 start_line, const char *code);
void perf_section_end(PerfCounter *counter);
void perf_print();

#define PERF_TIMED_STATEMENT(statement)                                                                                \
    {                                                                                                                  \
        PerfCounter *current_perf_counter = perf_section_start(__COUNTER__, __FUNCTION__, __LINE__, #statement);       \
        { statement; }                                                                                                 \
