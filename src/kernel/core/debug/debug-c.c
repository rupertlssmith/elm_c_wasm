...
void print_value(void *p) {
    ...
    switch (v->header.tag) {
        ...
        case Tag_Closure: {
            safe_printf("Closure (%s) n_values: %d max_values: %d values: ", Debug_evaluator_name(v->closure.evaluator),
...
void print_value_line(void *p) {
    ElmValue *v = p;
    safe_printf("| " FORMAT_PTR " | " FORMAT_HEX " |  %c   |%5d | ", v, *((size_t *) v), is_marked(v) ? 'X' : ' ',
                v->header.size);
    print_value(v);
    safe_printf("\n");
}
    ...
    switch (v->header.tag) {
        ...
        case Tag_Task: {
            ...
            static const char *task_ctors[] = {
                    "SUCCEED", "FAIL", "BINDING", "AND_THEN", "ON_ERROR", "RECEIVE",
            };
