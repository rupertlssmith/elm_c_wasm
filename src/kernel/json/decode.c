...
void *eval_Json_succeed(void *args[]) { return newCustom(DECODER_SUCCEED, 1, args); }
...
void *eval_Json_fail(void *args[]) { return newCustom(DECODER_FAIL, 1, args); }
Closure Json_fail = {
        .header = HEADER_CLOSURE(0),
        .max_values = 1,
...
void *eval_Json_decodeField(void *args[]) {
    void *field = args[0];
    void *decoder = args[1];
    return newCustom(DECODER_FIELD, 4,
                     ((void *[]){
                             /*a*/ &Json_null,
                             /*b*/ decoder,
                             /*c*/ &Json_null,
...
void *eval_Json_decodeIndex(void *args[]) {
    void *index = args[0];
    void *decoder = args[1];
    return newCustom(DECODER_INDEX, 5,
                     ((void *[]){
                             /*a*/ &Json_null,
                             /*b*/ decoder,
                             /*c*/ &Json_null,
...
void *eval_Json_andThen(void *args[]) {
    ...
    void *decoder = args[1];
    return newCustom(DECODER_AND_THEN, 8,
                     ((void *[]){
                             /*a*/ &Json_null,
                             /*b*/ decoder,
                             /*c*/ &Json_null,
...
void *eval_Json_oneOf(void *args[]) {
    ...
    return newCustom(DECODER_ONE_OF, 7,
                     ((void *[]){
                             /*a*/ &Json_null,
                             /*b*/ &Json_null,
                             /*c*/ &Json_null,
...
void *eval_Json_mapMany(void *args[]) {
    ...
    return newCustom(DECODER_MAP, 7,
                     ((void *[]){
                             /*a*/ &Json_null,
                             /*b*/ &Json_null,
                             /*c*/ &Json_null,
...
ElmString str_err_Bool = {.header = HEADER_STRING(6), .words16 = {'a', ' ', 'B', 'O', 'O', 'L'}};
ElmString str_err_Int = {.header = HEADER_STRING(6), .words16 = {'a', 'n', ' ', 'I', 'N', 'T'}};
ElmString str_err_Float = {.header = HEADER_STRING(7), .words16 = {'a', ' ', 'F', 'L', 'O', 'A', 'T'}};
ElmString str_err_String = {.header = HEADER_STRING(8), .words16 = {'a', ' ', 'S', 'T', 'R', 'I', 'N', 'G'}};
ElmString str_err_Null = {.header = HEADER_STRING(4), .words16 = {'n', 'u', 'l', 'l'}};
ElmString str_err_List = {.header = HEADER_STRING(6), .words16 = {'a', ' ', 'L', 'I', 'S', 'T'}};
ElmString str_err_Array = {.header = HEADER_STRING(8), .words16 = {'a', 'n', ' ', 'A', 'R', 'R', 'A', 'Y'}};
ElmString str_err_Object = {.header = HEADER_STRING(9), .words16 = {'a', 'n', ' ', 'O', 'B', 'J', 'E', 'C', 'T'}};
...
Custom *Json_runArrayDecoder(Custom *decoder, Custom *value, bool as_list) {
    ...
    for (u32 i = 0; i < len; i++) {
        ...
        if (RESULT_IS_OK(result) == &False) {
            return TAIL_RESULT_ERR(A2(&g_elm_json_Json_Decode_Index, newElmInt(i), result->values[0]));
        }
        array->values[i] = result->values[0];
    }

    void *elm_value = as_list ? List_create(len, array->values)
                              : A2(&g_elm_core_Array_initialize, newElmInt(len),
    ...
    switch (decoder->ctor) {
        ...
        case DECODER_LIST:
            ...
            if (value->header.tag == Tag_Custom && value->custom.ctor == JSON_VALUE_ARRAY) {
                return Json_runArrayDecoder(decoder->values[JsonField_decoder], &value->custom, true);
            }
        ...
        case DECODER_ARRAY: {
            ...
            if (value->header.tag == Tag_Custom && value->custom.ctor == JSON_VALUE_ARRAY) {
                return Json_runArrayDecoder(decoder->values[JsonField_decoder], &value->custom, false);
            }
        ...
        case DECODER_FIELD: {
            ...
            if (field_value != NULL) {
                ...
                return RESULT_IS_OK(result) == &True
                               ? result
                               : TAIL_RESULT_ERR(A2(&g_elm_json_Json_Decode_Field, field, result->values[0]));
            } else {
                ElmString *msg = A2(&String_append, &str_err_Field, A2(&String_append, field, &str_err_backtick));
        ...
        case DECODER_INDEX: {
            ...
            if (too_short) {
                ElmString *msg =
                        A2(&String_append, &str_err_Index,
                           A2(&String_append, A1(&String_fromNumber, index),
                              A2(&String_append, &str_err_Index_but_only_see,
                                 A2(&String_append, A1(&String_fromNumber, newElmInt(len)), &str_err_Index_entries))));
                return Json_expecting(msg, value);
            }
            ...
            return RESULT_IS_OK(result) == &True
                           ? result
                           : TAIL_RESULT_ERR(A2(&g_elm_json_Json_Decode_Index, index, result->values[0]));
        }
        ...
        case DECODER_KEY_VALUE: {
            ...
            for (i32 i = n_params - 2; i >= 0; i -= 2) {
                ...
                Custom *result = Json_runHelp(decoder->values[JsonField_decoder], object->values[i + 1]);
                if (RESULT_IS_OK(result) == &False) {
                    return TAIL_RESULT_ERR(A2(&g_elm_json_Json_Decode_Field, key, result->values[0]));
                }
        ...
        case DECODER_AND_THEN: {
            ...
            return RESULT_IS_OK(result) == &False
                           ? result
                           : Json_runHelp(A1(decoder->values[JsonField_callback], result->values[0]), value);
        }
        ...
        case DECODER_ONE_OF: {
            ...
            return TAIL_RESULT_ERR(A1(&g_elm_json_Json_Decode_OneOf, A1(&g_elm_core_List_reverse, errors)));
        }

        case DECODER_FAIL: {
            return TAIL_RESULT_ERR(A2(&g_elm_json_Json_Decode_Failure, decoder->values[JsonField_msg], WRAP(value)));
        }
...
void *eval_runOnString(void *args[]) {
    ...
    if (json == NULL) {
        return TAIL_RESULT_ERR(A2(&g_elm_json_Json_Decode_Failure, &str_invalid_json, WRAP(string)));
    }
...
void *EMSCRIPTEN_KEEPALIVE get_eval_Json_run() { return &eval_Json_run; }
