...
void test_heap_layout() {
    ...
    for (size_t kb = 16; kb <= 1024; kb *= 2) {
        ...
        bool bitmap_ok = (sizeof(void *) == sizeof(u64)) ? (percent_bitmap > 1.5 && percent_bitmap < 1.6)
        ...
        if (!bitmap_ok || !offsets_ok) {
            tests_failed++;
            safe_printf("FAIL: GC overhead should be the right fraction of the heap at %zu kB\n", kb);
            safe_printf("bitmap %f %%\n", percent_bitmap);
            safe_printf("gc_temp %f %%\n", percent_temp);
        } else if (verbose) {
            safe_printf("PASS: GC overhead should be the right fraction of the heap at %zu kB\n", kb);
        }
    }
...
void test_memcpy() {
    ...
    for (size = 1; size <= 6; ++size) {
        ...
        stbsp_snprintf(description, sizeof(description), "should correctly copy %zd 32-bit words, 32-bit aligned",
                       size);
    ...
    for (size = 1; size <= 6; ++size) {
        ...
        stbsp_snprintf(description, sizeof(description), "should correctly copy %zd 32-bit words, 64-bit aligned",
                       size);
        mu_assert(description, mismatches == 0);
    }
}
...
tce_loop:;
    do {
        if (iterations->value == 0) {
            ...
            GC_stack_tailcall(5, liveChunkSize, garbageChunkSize1, garbageChunkSize2, iterations, liveList);
            goto tce_loop;
        };
    } while (0);
}
...
void minor_gc_scenario(char *test_name, f32 fill_factor, i32 liveChunkSize, i32 garbageChunkSize1,
    ...
    i32 iterations_to_fill_heap = heap_size / (garbageChunkSize1 + garbageChunkSize2 + liveChunkSize);
    ...
    Closure *run = newClosure(4, 4, eval_generateHeapPattern,
                              ((void *[]){
