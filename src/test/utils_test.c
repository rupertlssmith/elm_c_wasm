...
void test_records() {
    if (verbose) {
        ...
        safe_printf("type alias Record1 =\n"
                    "    { someField: Int\n"
                    "    , otherField: String\n"
                    "    }\n"
                    "\n"
    ...
    */
    ...
    mu_assert("Updated record r3 should have same fieldgroup as r2", r3->fieldgroup == r2->fieldgroup);
...
void test_apply(void) {
    ...
    Closure user_project_closure = (Closure){
            .header = HEADER_CLOSURE(0), .evaluator = &eval_user_project_closure, .max_values = 3, .n_values = 0};
    ...
    if (verbose) {
        safe_printf("Example Elm function exercising most code paths in `apply`:\n"
                    "    outerScopeValue : Int\n"
                    "    outerScopeValue =\n"
                    "        1\n"
                    "\n"
        ...
        safe_printf("outerScopeValue addr=%p ctor=%d value=%d, hex=%s\n", &outerScopeValue,
                    (i64) outerScopeValue.header.tag, (i64) outerScopeValue.value,
                    hex(&outerScopeValue, sizeof(ElmInt)));

        safe_printf("two addr=%p ctor=%d value=%d, hex=%s\n", &two, (i64) two.header.tag, (i64) two.value,
                    hex(&two, sizeof(ElmInt)));

        safe_printf("three addr=%p ctor=%d value=%d, hex=%s\n", &three, (i64) three.header.tag, (i64) three.value,
                    hex(&three, sizeof(ElmInt)));

        safe_printf("closure addr=%p n_values=%d max_values=%d, hex=%s\n", closure, (i64) closure->n_values,
                    (i64) closure->max_values, hex(closure, sizeof(Closure) + closure->n_values * sizeof(void *)));

        safe_printf("curried addr=%p n_values=%d max_values=%d, hex=%s\n", curried, (i64) curried->n_values,
                    (i64) curried->max_values, hex(curried, sizeof(Closure) + curried->n_values * sizeof(void *)));

        safe_printf("answer addr=%p ctor=%d value=%d, hex=%s\n", answer, (i64) answer->header.tag, (i64) answer->value,
...
void test_eq(void) {
    ...
    mu_assert("Expect: \"hello\" == \"hello\" (by reference)", A2(&Utils_equal, hello1, hello1) == &True);
    mu_assert("Expect: \"hello\" == \"hello\" (by value)", A2(&Utils_equal, hello1, hello2) == &True);
    ...
    mu_assert("Expect: (2,3) == (2,3) (by value)", A2(&Utils_equal, tuple23, tuple23a) == &True);
    ...
    mu_assert("Expect: (1,2,3) == (1,2,3) (by ref)", A2(&Utils_equal, tuple123, tuple123) == &True);
    mu_assert("Expect: (1,2,3) == (1,2,3) (by value)", A2(&Utils_equal, tuple123, tuple123a) == &True);
    ...
    mu_assert("Expect: [1,1,1, ... ,1] == [1,1,1, ... ,1]", A2(&Utils_equal, bigList1, bigList2) == &True);
    mu_assert("Expect: [1,1,1, ... ,1] /= [1,1,1, ... ,2]", A2(&Utils_equal, bigList1, bigList3) == &False);
    ...
    mu_assert("Expect: Ctor1 1 'A' == Ctor1 1 'A' (ref)", A2(&Utils_equal, custom_1_1A, custom_1_1A) == &True);
    mu_assert("Expect: Ctor1 1 'A' == Ctor1 1 'A' (value)", A2(&Utils_equal, custom_1_1A, custom_1_1A_clone) == &True);
    mu_assert("Expect: Ctor1 1 'A' /= Ctor2 1 'A'", A2(&Utils_equal, custom_1_1A, custom_2_1A) == &False);
    mu_assert("Expect: Ctor1 1 'A' /= Ctor1 2 'A'", A2(&Utils_equal, custom_1_1A, custom_1_2A) == &False);
    mu_assert("Expect: Ctor1 1 'A' /= Ctor1 1 'B'", A2(&Utils_equal, custom_1_1A, custom_1_1B) == &False);
    ...
    mu_assert("Expect: {a=1, b=2} == {a=1, b=2} (ref)", A2(&Utils_equal, rec12, rec12) == &True);
    mu_assert("Expect: {a=1, b=2} == {a=1, b=2} (value)", A2(&Utils_equal, rec12, rec12a) == &True);
...
void test_compare() {
    ...
    mu_assert("compare: 123.456 == 123.456 (value)", A2(&Utils_compare, f1, f1a) == &g_elm_core_Basics_EQ);
    ...
    ElmChar a2 = (ElmChar){.header = HEADER_CHAR, .value = 'A'};
    ElmChar b = (ElmChar){.header = HEADER_CHAR, .value = 'B'};
    mu_assert("Expect: 'A' == 'A', by reference", A2(&Utils_compare, &a1, &a1) == &g_elm_core_Basics_EQ);
    ...
    mu_assert("Expect: (2,3) == (2,3) (by ref)", A2(&Utils_compare, tuple23, tuple23) == &g_elm_core_Basics_EQ);
    mu_assert("Expect: (2,3) == (2,3) (by value)", A2(&Utils_compare, tuple23, tuple23a) == &g_elm_core_Basics_EQ);
    ...
    mu_assert("Expect: (1,2,3) == (1,2,3) (by ref)", A2(&Utils_compare, tuple123, tuple123) == &g_elm_core_Basics_EQ);
    mu_assert("Expect: (1,2,3) == (1,2,3) (by value)",
              A2(&Utils_compare, tuple123, tuple123a) == &g_elm_core_Basics_EQ);
    mu_assert("Expect: (1,1,1) < (2,1,1)", A2(&Utils_compare, tuple111, tuple211) == &g_elm_core_Basics_LT);
    mu_assert("Expect: (1,1,1) < (1,2,1)", A2(&Utils_compare, tuple111, tuple121) == &g_elm_core_Basics_LT);
    mu_assert("Expect: (1,1,1) < (1,1,2)", A2(&Utils_compare, tuple111, tuple112) == &g_elm_core_Basics_LT);
    mu_assert("Expect: (2,1,1) > (1,1,1)", A2(&Utils_compare, tuple211, tuple111) == &g_elm_core_Basics_GT);
    mu_assert("Expect: (1,2,1) > (1,1,1)", A2(&Utils_compare, tuple121, tuple111) == &g_elm_core_Basics_GT);
    mu_assert("Expect: (1,1,2) > (1,1,1)", A2(&Utils_compare, tuple112, tuple111) == &g_elm_core_Basics_GT);
    ...
    mu_assert("Expect: [2] == [2] (by value)", A2(&Utils_compare, cons2, cons2a) == &g_elm_core_Basics_EQ);
    ...
    mu_assert("Expect: [2,3] == [2,3] (by ref)", A2(&Utils_compare, cons23, cons23) == &g_elm_core_Basics_EQ);
    mu_assert("Expect: [2,3] == [2,3] (by value)", A2(&Utils_compare, cons23, cons23a) == &g_elm_core_Basics_EQ);
