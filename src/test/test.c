...
#else
void wrapper_test() { tests_run--; }
void *eval_createTuple3(void *args[]) { return NULL; }
#endif
...
char *hex(void *addr, int size) {
    ...
    for (i = 0; i < size; c += 9, i += 4) {
        ...
        stbsp_sprintf(hex_strings[rotate] + c, "%02x%02x%02x%02x|", *((u8 *) addr + i), *((u8 *) addr + i + 1),
                      *((u8 *) addr + i + 2), *((u8 *) addr + i + 3));
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < ARRAY_LEN(suites); i++) {
        ...
        long_options[2 + i] = (struct option){suite->name, no_argument, NULL, suite->shortName};
    }
