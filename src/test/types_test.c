...
void test_elm_constants() {
    if (verbose) {
        ...
        safe_printf("Unit size=%zd addr=%p hex=%s\n", sizeof(Unit), &Unit, hex(&Unit, sizeof(Unit)));
        safe_printf("True size=%zd addr=%p hex=%s\n", sizeof(True), &True, hex(&True, sizeof(True)));
        safe_printf("False size=%zd addr=%p hex=%s\n", sizeof(False), &False, hex(&False, sizeof(False)));
    }
...
void test_cons() {
    ...
    if (verbose)
        safe_printf("Cons size=%zd addr=%p header.size=%d head=%s tail=%s, hex=%s\n", sizeof(Cons), c,
                    (int) c->header.size, c->head, c->tail, hex(c, sizeof(Cons)));

#if TARGET_64BIT
    mu_assert("Cons struct should be the right size for a header, 4 bytes of padding, and 2 "
              "pointers",
...
#if TARGET_64BIT
    mu_assert("Tuple2 struct should be the right size for a header, 4 bytes of padding, and 2 "
              "pointers",
...
#if TARGET_64BIT
    mu_assert("Tuple3 struct should be the right size for a header, 4 bytes of padding, and 3 "
              "pointers",
...
void test_float() {
    ...
    mu_assert("ElmFloat struct should be the right size for a header, 4 bytes of alignment "
              "padding, and an f64",
...
void test_record() {
    ...
    mu_assert("HEADER_RECORD macro should insert correct size field",
              r->header.size == 4); // 4*1
#endif
    mu_expect_equal("HEADER_RECORD macro should insert correct tag field", r->header.tag, Tag_Record);
}
...
void test_closure() {
    ...
    mu_assert("Closure struct with no values should be the size of a Header, an integer, and a "
              "pointer",
