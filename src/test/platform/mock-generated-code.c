...
void *createManager_Browser_Events() {
    return Platform_createManager(mockTask("Browser_Events_init"),
                                  ...
                                  mockFunction(3, mockTask("Browser_Events_onSelfMsg")), NULL,
                                  mockFunction(2, mockTask("Browser_Events_subMap")));
}

void *createManager_Http() {
    return Platform_createManager(mockTask("Http_init"), mockFunction(4, mockTask("Http_onEffects")),
                                  mockFunction(3, mockTask("Http_onSelfMsg")), mockFunction(2, mockTask("Http_cmdMap")),
                                  mockFunction(2, mockTask("Http_subMap")));
}

void *createManager_Task() {
    return Platform_createManager(mockTask("Task_init"), mockFunction(3, mockTask("Task_onEffects")),
                                  mockFunction(3, mockTask("Task_onSelfMsg")), mockFunction(2, mockTask("Task_cmdMap")),
                                  NULL);
}
...
void *ginit_author_project_Main_main() {
    Closure *init =
            mockFunction(1, newTuple2(create_string("init"), A1(&g_elm_core_Task_command, create_string("init Cmd"))));
    Closure *subs = mockFunction(
            1, A1(&Platform_batch, List_create(2, (void *[]){
    ...
    Closure *update = mockFunction(
            2, newTuple2(create_string("update"), A1(&g_elm_http_Http_command, create_string("update Cmd"))));
    Closure *view = mockFunction(0xffff, create_string("view"));
    Record *elmTeaRecord = newRecord(&fg_init_subscriptions_update_view, 4, ((void *[]){init, subs, update, view}));
...
int mock_app_main() { return GC_init(); }