...
int ftest(ElmFloat *actual, f64 expected) { return fabs(actual->value - expected) / expected < 0.001; }

int itest(ElmInt *actual, i32 expected) { return (i32) actual->value == expected; }
...
void test_number_binops() {
    ...
    mu_assert("Expect: 1.0 + (-123.456) = -122.456", ftest(A2(&Basics_add, f1, fm), -122.456));
    ...
    mu_assert("Expect: 123456-654321=-530865", itest(A2(&Basics_sub, ibig1, ibig2), -530865));

    mu_assert("Expect: 1.0 - 3.14 = -2.14", ftest(A2(&Basics_sub, f1, f2), -2.14));
    mu_assert("Expect: 1.0 - (-123.456) = 124.456", ftest(A2(&Basics_sub, f1, fm), 124.456));
    ...
    mu_assert("Expect: 3.14 * (-123.456) = -387.65184", ftest(A2(&Basics_sub, f1, fm), -387.65184));
    ...
    mu_assert("Expect: -654321 // -1 = 654321", itest(A2(&Basics_idiv, imbig2, im), 654321));
...
void test_number_unops() {
    ...
    // Basics.toFloat
    mu_assert("toFloat -123 == -123.0", ftest(A1(&Basics_toFloat, newElmInt(-123)), -123.0));
