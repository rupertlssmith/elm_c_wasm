...
static void init_test_values() {
    ...
    Tuple3 *tuple3_XYZ = newTuple3(create_string("X"), create_string("Y"), create_string("Z"));
    Cons *list123 = List_create(3, (void *[]){newElmInt(1), newElmInt(2), newElmInt(3)});
    Record *knownRecord = newRecord(&fg_init_subscriptions_update_view, 4,
                                    (void *[]){
  ...
  Record* unknownRecord = newRecord(unknownFieldGroup,
    ...
    Custom *kernelUnion = newCustom(KERNEL_CTOR_OFFSET + 5, 5,
                                    (void *[]){
                                            /* a */ NULL,
                                            /* b */ str_kernel,
                                            /* c */ NULL,
...
void *eval_createTuple3(void *args[]) { return newTuple3(args[0], args[1], args[2]); }
...
static void test_call_wasm_from_js() {
    ...
    Tuple3 *expected = A2(c, newElmInt(999), A1(&g_elm_core_Maybe_Just, create_string("hello")));
    expect_equal("should correctly apply 2 JS arguments to a Wasm closure", actual, expected);
}


static void test_call_js_from_wasm() {
    JsRef *jsKernelIncrementBy1 = testWriteJsCallbackToWasm();
    mu_expect_equal("JS function should appear as a JsRef when written to Wasm", jsKernelIncrementBy1->header.tag,
                    Tag_JsRef);
