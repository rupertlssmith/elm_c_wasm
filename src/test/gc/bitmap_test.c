...
void expect_alloc(char *message, size_t *expected_start, size_t *expected_end, size_t *actual_start,
                  size_t *actual_end) {
    assertions_made++;
    if (actual_start != expected_start || actual_end != expected_end) {
        tests_failed++;
        safe_printf("FAIL: %s\n  expected: 0x%zx -> 0x%zx\n    actual: 0x%zx -> 0x%zx\n", message, expected_start,
                    expected_end, actual_start, actual_end);
    } else if (verbose) {
        safe_printf("PASS: %s\n", message);
    }
}
...
void test_bitmap_find_space() {
    ...
    expect_alloc("should find entire heap when it is empty", heap->start, heap->end, alloc, end_of_space);
    ...
    alloc = bitmap_find_space(heap, heap->start, 1, &end_of_space);
    expect_alloc("should find a 64-word space at the start", heap->start, heap->start + 64, alloc, end_of_space);
    ...
    alloc = bitmap_find_space(heap, heap->start, 1, &end_of_space);
    expect_alloc("should find a 64-word space after a 64-word live section", heap->start + 64, heap->start + 128, alloc,
                 end_of_space);
    ...
    alloc = bitmap_find_space(heap, heap->start, 1, &end_of_space);
    expect_alloc("should skip over a 60-word space", heap->start + 192, heap->end, alloc, end_of_space);
    ...
    alloc = bitmap_find_space(heap, heap->start, 1, &end_of_space);
    expect_alloc("should skip over one space and one allocation", heap->start + ALLOC_CHUNK, heap->end, alloc,
                 end_of_space);
    ...
    alloc = bitmap_find_space(heap, heap->start, 1, &end_of_space);
    expect_alloc("should find space in the last word of the bitmap", last_word, heap->end, alloc, end_of_space);
    ...
    expect_alloc("should find a 64-word space just before the end of the heap", before_last_word, last_word, alloc,
                 end_of_space);
    ...
    expect_alloc("should be able to expand into free space in the first and last bitmap words",
                 heap->start + ALLOC_CHUNK, (size_t *) last_chunk, alloc, end_of_space);
}
...
void perf_bitmap_find_space() {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        space_old = 0;
        for (size_t *end_of_alloc_patch = heap->start, *alloc = heap->start; alloc;
             alloc = bitmap_find_space_old(heap, end_of_alloc_patch, 0, &end_of_alloc_patch)) {
