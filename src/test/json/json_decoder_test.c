...
void test_decode_err(Closure *runner, Custom *decoder, char *json_c_str, Custom *expected_err) {
...
void test_decode_errFailure(Closure *runner, Custom *decoder, char *json_c_str, char *msg_c_str) {
    ElmString *json = create_string(json_c_str);
    stbsp_sprintf(test_decode_buf, "should return expected error for '%s'", json_c_str);

    void *actual_err = runner == &Json_runOnString ? A2(&Json_runOnString, decoder, json)
                                                   : A2(&Json_run, decoder, createJsValue(json));

    void *expected_err = newCustom(CTOR_Err, 1,
                                   ((void *[]){
                                           newCustom(CTOR_Failure, 2,
                                                     ((void *[]){
...
Custom *errIndex(u32 idx, Custom *error) { return newCustom(CTOR_Index, 2, ((void *[]){newElmInt(idx), error})); }
Custom *errOneOf(Cons *errors) { return newCustom(CTOR_OneOf, 1, (void *[]){errors}); }
Custom *errFailure(char *msg_c_str, void *json_value) {
    return newCustom(CTOR_Failure, 2,
                     ((void *[]){
                             create_string(msg_c_str),
                             WRAP(json_value),
                     }));
}
Custom *err(Custom *error) { return newCustom(CTOR_Err, 1, (void *[]){error}); }
...
void test_Json_decode_invalidJson() {
    ...
    Custom *expectedError = err(newCustom(CTOR_Failure, 2,
                                          ((void *[]){
...
void test_Json_decodeValue(void *runner) {
    void *expected = runner == &Json_runOnString ? (void *) newCustom(JSON_VALUE_ARRAY, 3,
                                                                      ((void *[]){
                                                                              newElmFloat(1),
                                                                              newElmFloat(2),
                                                                              newElmFloat(3),
...
void test_Json_decodeList(void *runner) {
    ...
    test_decode_ok(runner, decoder, "[1,2,3]",
                   List_create(3, ((void *[]){
                                          newElmInt(1),
                                          newElmInt(2),
                                          newElmInt(3),
    ...
    test_decode_err(runner, decoder, "[null]", err(errIndex(0, errFailure("Expecting an INT", &Json_null))));
}

void test_Json_decodeArray(void *runner) {
    Custom *decoder = A1(&Json_decodeArray, &Json_decodeInt);

    test_decode_ok(runner, decoder, "[0,1,2]", A2(&g_elm_core_Array_initialize, newElmInt(3), &Basics_identity));
...
void test_Json_decodeField(void *runner) {
    ...
    test_decode_ok(runner, decoder, "{ \"a\": null, \"myField\": 123, \"b\": null }", newElmInt(123));

    test_decode_errFailure(runner, decoder, "null", "Expecting an OBJECT with a field named `myField`");

    test_decode_err(runner, decoder, "{\"myField\":null}",
...
void test_Json_decodeIndex(void *runner) {
    ...
    test_decode_err(runner, decoder, "[123,456]",
                    err(errFailure("Expecting a LONGER array. Need index 2 but only see 2 entries", too_short)));

    test_decode_err(runner, decoder, "[{},\"hello\",null,123]",
...
void test_Json_decodeKeyValuePairs(void *runner) {
    ...
    test_decode_ok(runner, decoder, "{ \"a\": 1, \"b\": 2, \"c\": 3 }",
                   List_create(3, ((void *[]){
    ...
    test_decode_err(runner, decoder, "{ \"a\": 1, \"b\": null, \"c\": 3 }",
...
void test_Json_map(void *runner) {
    Closure *mapFns[] = {
            &Json_map1, &Json_map2, &Json_map3, &Json_map4, &Json_map5, &Json_map6, &Json_map7, &Json_map8,
    };
...
void test_Json_andThen(void *runner) {
    ...
    */
    Custom *decoder =
            A2(&Json_andThen, &selectField, A2(&Json_decodeField, create_string("dataField"), &Json_decodeString));

    test_decode_ok(runner, decoder, "{ \"dataField\": \"foo\", \"foo\": 123 }", newElmInt(123));

    test_decode_ok(runner, decoder, "{ \"dataField\": \"bar\", \"bar\": 456 }", newElmInt(456));

    test_decode_errFailure(runner, decoder, "null", "Expecting an OBJECT with a field named `dataField`");

    test_decode_err(runner, decoder, "{ \"dataField\": null }",
                    err(errField("dataField", errFailure("Expecting a STRING", &Json_null))));

    test_decode_err(runner, decoder, "{ \"dataField\": \"foo\", \"foo\": null }",
...
void test_Json_oneOf(void *runner) {
    ...
    */
    ...
    Custom *decoder = A1(&Json_oneOf, List_create(2, ((void *[]){
                                                             &Json_decodeInt,
    ...
    void *empty_array = runner == &Json_runOnString ? (void *) newCustom(JSON_VALUE_ARRAY, 0, NULL)
                                                    : (void *) newJsRef(latest_jsref_index + 1);

    test_decode_err(runner, decoder, "[]",
                    err(errOneOf(List_create(2, ((void *[]){
...
void test_Json_fail(void *runner) {
    ...
    if (runner == &Json_runOnString) {
        ...
        test_decode_err(runner, decoder, "[1,2,3]", err(errFailure(msg, newJsRef(latest_jsref_index + 1))));
    }
}
