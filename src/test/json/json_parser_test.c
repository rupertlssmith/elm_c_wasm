...
void parse_test(void *(*parse_func)(u16 **cursor, u16 *end), ElmString *json, void *expect_val,
    ...
    if (expect_val) {
        msg_prefix_len =
                stbsp_sprintf(parse_test_msg, "should correctly parse %zd chars from '", expect_chars_consumed);
    } else {
...
void test_Json_parse_string() {
    // valid strings
    parse_test(&parse_string, create_string(DQUOTE "hello" DQUOTE), create_string("hello"), 7);

    parse_test(&parse_string, create_string(DQUOTE "hello" DQUOTE ": 123"), create_string("hello"), 7);
    ...
    parse_test(&parse_string, create_string(DQUOTE "\\uD852\\uDF62" DQUOTE), &unicode_test_string, 14);
...
void test_Json_parse_array() {
    ...
    parse_test(&parse_array, create_string("[123]"), newCustom(JSON_VALUE_ARRAY, 1, (void *[]){newElmFloat(123)}), 5);

    parse_test(&parse_array, create_string("[\r\n\tnull\r\n ] "),
               newCustom(JSON_VALUE_ARRAY, 1, (void *[]){&Json_null}), 12);

    parse_test(&parse_array, create_string("[ 123,\"hi\" ]"),
               newCustom(JSON_VALUE_ARRAY, 2,
                         ((void *[]){
                                 newElmFloat(123),
                                 create_string("hi"),
                         })),
...
void test_Json_parse_object() {
    ...
    parse_test(&parse_object, create_string("{\"a\":123}"),
               newCustom(JSON_VALUE_OBJECT, 2,
                         ((void *[]){
                                 create_string("a"),
                                 newElmFloat(123),
                         })),
    ...
    parse_test(&parse_object, create_string("{ \"a\":123,\"stuff\":false }"),
               newCustom(JSON_VALUE_OBJECT, 4,
                         ((void *[]){
                                 create_string("a"),
                                 newElmFloat(123),
                                 create_string("stuff"),
    ...
    parse_test(&parse_object, create_string("{}"), newCustom(JSON_VALUE_OBJECT, 0, NULL), 2);

    parse_test(&parse_object, create_string("{ \r\n\t}"), newCustom(JSON_VALUE_OBJECT, 0, NULL), 6);
