...
void test_Json_encode_boolean() {
    expect_equal("should encode True to 'true'", A2(&Json_encode, &int0, WRAP(&True)), create_string("true"));
    expect_equal("should encode False to 'false'", A2(&Json_encode, &int0, WRAP(&False)), create_string("false"));
}

void test_Json_encode_null() {
    expect_equal("should encode null to 'null'", A2(&Json_encode, &int0, WRAP(&Json_null)), create_string("null"));
}

void test_Json_encode_number() {
    expect_equal("should encode 3.14 to '3.14'", A2(&Json_encode, &int0, WRAP(newElmFloat(3.14))),
                 create_string("3.14"));

    expect_equal("should encode 1e22 to '1e+22'", A2(&Json_encode, &int0, WRAP(newElmFloat(1e22))),
                 create_string("1e+22"));

    expect_equal("should encode `123 : Int` to '123'", A2(&Json_encode, &int0, WRAP(newElmInt(123))),
                 create_string("123"));
}

void test_Json_encode_string() {
    expect_equal("should encode 'hello' to '\"hello\"'", A2(&Json_encode, &int0, WRAP(create_string("hello"))),
                 create_string("\"hello\""));

    expect_equal("should encode "
                 "' \\ \" \\r \\n \\t \\b \\f '"
                 " to "
                 "'\" \\\\ \\\" \\r \\n \\t \\b \\f \"'",
    ...
    expect_equal("should encode ASCII control chars 0x7 and 0xb to '\"\\u0007\\u000b\"'",
                 A2(&Json_encode, &int0, WRAP(create_string("\a\v"))), create_string("\"\\u0007\\u000b\""));
}
...
void test_Json_stringify_array() {
    ...
    Custom *array123 = newCustom(JSON_VALUE_ARRAY, 3,
                                 ((void *[]){
                                         newElmFloat(1),
                                         newElmFloat(2),
                                         newElmFloat(3),
    ...
    expect_equal("encode 0 [1,2,3] == \"[1,2,3]\"", A2(&Json_encode, &int0, WRAP(array123)), create_string("[1,2,3]"));
    ...
    expect_equal(buf, A2(&Json_encode, newElmInt(2), WRAP(array123)), create_string(expected));
    ...
    stbsp_sprintf(buf, "encode 2 [[1,2,3]] = \n%s", expected);
    expect_equal(buf,
                 A2(&Json_encode, newElmInt(2),
                    WRAP(newCustom(JSON_VALUE_ARRAY, 2,
                                   ((void *[]){
                                           array123,
                                           array123,
                                   })))),
...
void test_Json_stringify_object() {
    ...
    Custom *exampleObj = newCustom(JSON_VALUE_OBJECT, 6,
                                   ((void *[]){
                                           create_string("a"),
                                           newElmFloat(1),
                                           create_string("b"),
    ...
    expect_equal("encode 0 {a:1,b:2,c:3} == \"{\"a\":1,\"b\":2,\"c\":3}\"", A2(&Json_encode, &int0, WRAP(exampleObj)),
    ...
    expect_equal(buf, A2(&Json_encode, newElmInt(2), WRAP(exampleObj)), create_string(expected));
    ...
    expect_equal(buf,
                 A2(&Json_encode, newElmInt(2),
                    WRAP(newCustom(JSON_VALUE_OBJECT, 4,
                                   ((void *[]){
                                           create_string("x"),
                                           exampleObj,
                                           create_string("y"),
...
void *eval_elm_json_Json_Encode_list(void *args[]) {
    ...
    return A1(&Json_wrap,
              A3(&g_elm_core_List_foldl, A1(&Json_addEntry, x_func), A1(&Json_emptyArray, &Unit), x_entries));
}
...
void test_Json_Encode_list() {
    ...
    Custom *encoded = A2(&g_elm_json_Json_Encode_list, &g_elm_json_Json_Encode_bool, list_bool);

    expect_equal("Encode.list Encode.bool [True,False] == JsonValueArray True False", encoded, WRAP(json_array));

    expect_equal("encode 0 (list bool [True,False]) == \"[true,false]\"", A2(&Json_encode, &int0, encoded),
...
void *eval_elm_json_Json_Encode_object(void *args[]) {
    ...
    return A1(&Json_wrap, A3(&g_elm_core_List_foldl, tmp0, A1(&Json_emptyObject, &Unit), x_pairs));
}
...
void test_Json_encode_object() {
    ...
    Cons *elm_pairs = List_create(2, ((void *[]){
                                             newTuple2(name, A1(&g_elm_json_Json_Encode_string, brian)),
                                             newTuple2(age, A1(&g_elm_json_Json_Encode_int, &int42)),
                                     }));

    Custom *json_object = WRAP(newCustom(JSON_VALUE_OBJECT, 4, ((void *[]){name, brian, age, &float42})));

    void *encodedObject = A1(&g_elm_json_Json_Encode_object, elm_pairs);

    expect_equal("object [(\"name\", string \"Brian\"), (\"age\", int 42)]"
                 " == JsonValueObject \"name\" \"Brian\" \"age\" 42.0",
                 encodedObject, json_object);

    expect_equal("encode 0 (object [(\"name\", string \"Brian\"), (\"age\", int 42)])"
                 " == \"\"\"{\"name\":\"Brian\",\"age\":42}\"\"\"",
                 A2(&Json_encode, &int0, encodedObject), create_string("{\"name\":\"Brian\",\"age\":42}"));
}
