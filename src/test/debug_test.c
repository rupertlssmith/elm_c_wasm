...
void test_StringBuilder() {
    ...
    Closure *createFragmentation = newClosure(2, 2, eval_createFragmentation, (void *[]){&Nil, newElmInt(162)});
    void *liveList = GC_execute(createFragmentation);
    GC_register_root(&liveList);

    Closure *buildString = newClosure(1, 1, eval_buildString, (void *[]){newElmInt(6 * 164 * sizeof(void *))});
