...
void test_String_uncons() {
    ...
    expect_equal("uncons \"abc\" == Just ('a',\"bc\")", A1(&String_uncons, abc),
                 A1(&g_elm_core_Maybe_Just, newTuple2(a_char, bc)));

    expect_equal("uncons \"\" == Nothing", A1(&String_uncons, empty), &g_elm_core_Maybe_Nothing);

    expect_equal("uncons \"a\" == Just ('a',\"\")", A1(&String_uncons, a_str),
...
void test_String_append() {
    ...
    ElmString *empty = create_string("");
    expect_string("append \"hello\" \" world\"", "hello world", A2(&String_append, hello, world));
...
void test_String_split() {
    ...
    expect_equal("split \"abc\" \"ab\" == [\"ab\"]  # separator longer than string", A2(&String_split, abc, ab),
                 list_ab);

    expect_equal("split \"\" \"ab\" == [\"a\",\"b\"]  # split on empty string", A2(&String_split, empty, ab),
                 List_create(2, (void *[]){
                                        create_string("a"),
                                        create_string("b"),
                                }));

    expect_equal("split \"ab\" \"ab\" == [\"\",\"\"]", A2(&String_split, ab, ab),
                 List_create(2, (void *[]){
                                        empty,
                                        empty,
                                }));

    expect_equal("split \"ab\" \"\" == [\"\"]", A2(&String_split, ab, empty), newCons(empty, &Nil));
}

void test_String_join() {
    ElmString *slash = create_string("/");
    ElmString *empty = create_string("");
    expect_string("join \"/\" [\"home\",\"steve\",\"Desktop\"]", "home/steve/Desktop",
                  A2(&String_join, slash,
                     List_create(3, (void *[]){
                                            create_string("home"),
                                            create_string("steve"),
                                            create_string("Desktop"),
...
void test_String_slice() {
    ...
    expect_string("slice 1 2 \"abc\"", "b", A3(&String_slice, newElmInt(1), newElmInt(2), abc));
    expect_string("slice 1 -1 \"abc\"", "b", A3(&String_slice, newElmInt(1), newElmInt(-1), abc));
    expect_string("slice -2 2 \"abc\"", "b", A3(&String_slice, newElmInt(-2), newElmInt(2), abc));
    expect_string("slice -2 -1 \"abc\"", "b", A3(&String_slice, newElmInt(-2), newElmInt(-1), abc));

    expect_string("slice 2 1 \"abc\"", "", A3(&String_slice, newElmInt(2), newElmInt(1), abc));

    expect_string("slice 0 3 \"abc\"", "abc", A3(&String_slice, newElmInt(0), newElmInt(3), abc));
    expect_string("slice -3 0 \"abc\"", "", A3(&String_slice, newElmInt(-3), newElmInt(0), abc));
    expect_string("slice  0 -3 \"abc\"", "", A3(&String_slice, newElmInt(0), newElmInt(-3), abc));

    expect_string("slice -100 100 \"abc\"", "abc", A3(&String_slice, newElmInt(-100), newElmInt(100), abc));
    expect_string("slice -100 100 \"\"", "", A3(&String_slice, newElmInt(-100), newElmInt(100), create_string("")));
}
...
void test_String_indexes() {
    ...
    expect_equal("indexes \"abc\" \"ab\" == []  # substring longer than string", A2(&String_indexes, abc, ab), &Nil);

    expect_equal("indexes \"\" \"ab\" == []  # empty substring", A2(&String_indexes, empty, ab), &Nil);

    expect_equal("indexes \"ab\" \"ab\" == [0]", A2(&String_indexes, ab, ab), newCons(newElmInt(0), &Nil));

    expect_equal("indexes \"ab\" \"\" == []", A2(&String_indexes, ab, empty), &Nil);
}

void test_String_trim() {
    expect_string("trim \" \\n a \\t \\r\\n\"", "a", A1(&String_trim, create_string(" \n a \t \r\n")));
...
void test_String_trimLeft() {
    ...
    expect_string("trimLeft \" \\n a\"", "a", A1(&String_trimLeft, create_string(" \n a")));
...
void test_String_trimRight() {
    expect_string("trimRight \" \\n a \\t \\r\\n\"", " \n a", A1(&String_trimRight, create_string(" \n a \t \r\n")));

    expect_string("trimRight \" \\n a\"", " \n a", A1(&String_trimRight, create_string(" \n a")));

    expect_string("trimRight \"a \\t \\r\\n\"", "a", A1(&String_trimRight, create_string("a \t \r\n")));
...
void test_String_all() {
    expect_equal("all ((==) 'X') \"XXXXX\" == True", A2(&String_all, &isX, create_string("XXXXX")), &True);

    expect_equal("all ((==) 'X') \"XXXX-\" == False", A2(&String_all, &isX, create_string("XXXX-")), &False);

    expect_equal("all ((==) 'X') \"\" == True", A2(&String_all, &isX, create_string("")), &True);
}
...
void test_String_contains() {
    ...
    expect_equal("contains \"abc\" \"\" == False", A2(&String_contains, abc, empty), &False);
...
void test_String_startsWith() {
    ...
    expect_equal("startsWith \"ab\" \"abc\" == True", A2(&String_startsWith, ab, abc), &True);

    expect_equal("startsWith \"bc\" \"abc\" == False", A2(&String_startsWith, bc, abc), &False);

    expect_equal("startsWith \"abc\" \"ab\" == False", A2(&String_startsWith, abc, ab), &False);

    expect_equal("startsWith \"abc\" \"\" == False", A2(&String_startsWith, abc, empty), &False);

    expect_equal("startsWith \"\" \"abc\" == True", A2(&String_startsWith, empty, abc), &True);

    expect_equal("startsWith \"\" \"\" == True", A2(&String_startsWith, empty, empty), &True);
}
...
void test_String_endsWith() {
    ...
    expect_equal("endsWith \"abc\" \"\" == False", A2(&String_endsWith, abc, empty), &False);
...
void test_String_fromNumber() {
    expect_string("fromNumber 2147483647", "2147483647", A1(&String_fromNumber, newElmInt(2147483647)));
    expect_string("fromNumber -2147483648", "-2147483648", A1(&String_fromNumber, newElmInt(-2147483648)));
    expect_string("fromNumber -2147483647", "-2147483647", A1(&String_fromNumber, newElmInt(-2147483647)));
    expect_string("fromNumber -1", "-1", A1(&String_fromNumber, newElmInt(-1)));
    expect_string("fromNumber -3.141592653589793", "-3.141592653589793",
                  A1(&String_fromNumber, newElmFloat(-3.141592653589793)));
    expect_string("fromNumber -3141592653589793", "-3141592653589793",
                  A1(&String_fromNumber, newElmFloat(-3141592653589793)));
}

void test_String_toInt() {
    expect_equal("toInt \"2147483647\" == Just 2147483647", A1(&String_toInt, create_string("2147483647")),
                 A1(&g_elm_core_Maybe_Just, newElmInt(2147483647)));

    expect_equal("toInt \"-2147483648\" == Just -2147483648", A1(&String_toInt, create_string("-2147483648")),
                 A1(&g_elm_core_Maybe_Just, newElmInt(-2147483648)));

    expect_equal("toInt \"+10\" == Just 10", A1(&String_toInt, create_string("+10")),
                 A1(&g_elm_core_Maybe_Just, newElmInt(10)));

    expect_equal("toInt \"0\" == Just 0", A1(&String_toInt, create_string("0")),
    ...
    expect_equal("toInt \"hello\" == Nothing", A1(&String_toInt, create_string("hello")), &g_elm_core_Maybe_Nothing);

    expect_equal("toInt \"\" == Nothing", A1(&String_toInt, create_string("")), &g_elm_core_Maybe_Nothing);
}
